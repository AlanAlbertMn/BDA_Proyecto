<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "INSERT INTO articulo (idArticulo,nombre,cantidad,precioPorUnidad,marca,proveedor,categoria) values(?, ?, ?, ?, ?, ?, ?)";			
			$q = $pdo->prepare($sql);
			$q->execute(array($idSano,$nombreSanitizado,$cantSanitizado,$precioSanitizado,$marcaSanitizado,$provSanitizado,$catSanitizado));
			Database::disconnect();
