<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE articulo set idArticulo = ?, nombre = ?, cantidad =?, precioPorUnidad=?, marca=?, proveedor=?, categoria=? WHERE idArticulo = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($idSano,$nombreSanitizado,$cantSanitizado,$precioSanitizado,$marcaSanitizado,$provSanitizado,$catSanitizado,$id));
			Database::disconnect();	
			header("Location: index.php");
		}
		} else {
